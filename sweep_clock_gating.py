...
"""
===============================================================================
门控时钟 (Clock Gating) 多维度参数化扫描与物理签核分析套件
...
功能说明:
  ...
  执行架构:
    - 物理实现（PnR）：对各寄存器规模仅需执行 1 次完整 LibreLane 80 阶段 PnR
...
def run_activity_sim_and_sta(
    ...
    ec.run_command_with_logging(compile_cmd, sim_log, cwd=sim_dir, logger=logger)

    # 2. 注入动态 +EN_DUTY 运行仿真
    run_cmd = ["vvp", str(sim_out), f"+VCD_FILE={vcd_out}", f"+EN_DUTY={duty}"]
    vvp_log = log_dir / f"sim_run_{sim_tag}.log"
    ec.run_command_with_logging(run_cmd, vvp_log, cwd=sim_dir, logger=logger)
    ...
    ec.run_command_with_logging(["sta", str(sta_cmd_path)], sta_log, cwd=reports_dir, logger=logger)
...
def main():
    ...
    sweep_workspace = Path(args.workspace).resolve()
    ...
    lib_path, verilog_lib_path, prims_path, blackbox_path = ec.locate_pdk_files(pdk_root_base, "sky130A", "sky130_fd_sc_hd")
    ...
    for w in widths:
        ...
        meta, srcs_orig, srcs_opt, tb_path = ec.load_and_validate_case(case_dir, logger)
        ...
        ec.run_formal_lec(srcs_orig, srcs_opt, top_module, w_formal_dir, w_log_dir, logger)
        ...
        for tag, srcs in [("orig", srcs_orig), ("opt", srcs_opt)]:
            ...
            if final_nl.exists() and final_spef.exists():
                ...
                netlist, spef, metrics_json = ec.run_pnr_flow(
    ...
    logger.info("=" * 80)
    logger.info(f"Clock Gating Parametric Sweep completed successfully!")
