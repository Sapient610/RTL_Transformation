...
def generate_oi_study_report(
    ...
    lines.extend([
        ...
        "![不同数据总线翻转活跃度对操作数隔离功耗优化率的影响对比](./images/operand_isolation/oi_activity_impact.svg)\n",
        "- **杂散翻转渗透放大**: 当外部数据总线在高频翻转（Activity = 60%）时，在未隔离设计中，大量伪随机跳变无休止地引发进位链和异或逻辑级联翻转；操作数隔离将其强行锁死在 0，截断了深层逻辑树的全部无效充放电，带来了最为惊人的动态功耗降幅（最高削减 -54.4%）；",
        "- **平缓总线下的收益折损**: 当数据总线本身保持稳定（Activity = 10%）时，即使不隔离，深层逻辑翻转密度也很低。此时插入隔离门所节省的动态功耗相对有限，其自身引入的寄生开关电容反而容易占上风；",
        "- **收支平衡逆转拐点（活动率的决定性杠杆）**: 在稀疏计算工况（Valid=20%）下，当总线翻转率由 10% 增至 60% 时，所有位宽（8b/16b/32b/64b）均经历了一次戏剧性的**损益状态反转**——从 10% 时的全线负优化（+0.87% ~ +5.67%，隔离门开销反噬）彻底扭转为 60% 时的显著正收益（-19.17% ~ -24.71%）。这充分证明：**数据总线翻转活跃度是操作数隔离技术的生命线，对于静态或低噪总线盲目插入隔离门将招致负优化，而对于高动态总线则能撬动数倍的节能杠杆**。",
