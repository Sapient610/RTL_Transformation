...
> **物理异常提示**：8-Tap FIR 的标准单元增量（+81 门）反常高于 12-Tap（+63 门）与 16-Tap（+67 门），且走线总长激增 +17.6%（净增超 4000 um），其诱因为逻辑综合技术映射中的门控边界溶解与后端版图高扇出交叉绕线膨胀，详见 3.4 节深度剖析。
...
#### 3. 微观差异二：为什么「4-Tap > 8-Tap」？（8-Tap 逻辑门/面积异常增生与布局总线暴增根因剖析）

全物理签核实测揭示了一个高度反常的物理现象：4-Tap 优化版仅增加 20 门（+2.16% 面积），12-Tap 增加 63 门，16-Tap 增加 67 门；而 **8-Tap 优化版标准单元数反常暴增 +81 门**（面积净增 +256.5 $\mu\text{m}^2$），布局布线总长从 22,756 $\mu\text{m}$ 飙升至 26,760 $\mu\text{m}$（**走线总长暴增 +4,004 $\mu\text{m}$ / +17.60%**）。走线与版图膨胀导致时钟树动态功耗由 473 $\mu\text{W}$ 逆势上升至 510 $\mu\text{W}$（**时钟功耗反增 +37 $\mu\text{W} / +7.82\%$**），直接吞噬了乘法器门控省下的电量。

经深度提取 Yosys 综合网表、ABC 映射日志及 OpenROAD PnR 各阶段中间报告，该反常现象的根本机理涵盖以下三大协同诱因：

1. **逻辑综合与 ABC 技术映射的边界溶解与碎化分解 (Boundary Dissolution & Gate Fragmentation)**:
   - **设计意图 vs 映射行为**: RTL 意图仅在顶层输入端口施加 8-bit 与门阻断广播（理论仅需 8 个 `and2` 门）；
   - **常数加法树重散列**: 8-Tap FIR 的滤波系数（3, 11, 28, 54）被展开为移位加法树。由于未对门控插入点设置 `(* keep *)` 属性保护边界，ABC 面积驱动优化器（`AREA_0.abc` 中的 `amap`）在执行结构散列（strash）与因式分解时，识别出 `data_valid` 能够与乘法器内部加法算子进行公共因子因式分解（Sub-expression Factoring）；
   - **集中门控被溶解渗透**: ABC 并没有保持输入端的 8 个独立与门，而是将 `data_valid` 控制项下推并深度溶解进了所有 8 个乘法器内部的进位链与加法网络中；
   - **高密宏单元被拆碎为离散复合门**: 溶解导致原始设计中原本映射成高度紧凑专用宏单元的结构被破坏拆散——专用的 2 选 1 数据选择器减少了 **-47 个**（`mux2_1` 从 83 个骤降到 36 个），异或门减少了 **-40 个**（`xor2_2` 减少 -21 个，`xnor2_2` 减少 -19 个）。取而代之的是，工具生成了大量结构离散、引脚繁杂的小型复合逻辑门：
     * `sky130_fd_sc_hd__and3_2` (3输入与门) 暴增 **+54 个** (14 → 68 个)；
     * `sky130_fd_sc_hd__a21o_2` (2输入与后或门) 增加 **+29 个** (44 → 73 个)；
     * `sky130_fd_sc_hd__a21oi_2` 增加 **+19 个** (25 → 44 个)；
     * `sky130_fd_sc_hd__a22o_2` 增加 **+13 个** (8 → 21 个)；
     * `sky130_fd_sc_hd__inv_2` 反相器增加 **+11 个** (9 → 20 个)；
   - **门数净增 +78 门**: 逻辑碎片化重构直接造成 Yosys 综合单元数净增 **+78 门**（919 → 997 门），最终物理实现增量达 **+81 门**。相比之下，4-Tap 逻辑规模小未触发该重分解，而 12/16-Tap 乘法器阶数更多、加法树更深，ABC 难以跨级因式分解，故未出现同等碎化。

2. **版图膨胀与高扇出全局跨越导致布局布线暴增 (Floorplan Dilation & High-Fanout Cross-Routing)**:
   - **Floorplan Core 区域被迫扩张**: OpenLane 流程根据综合后标准单元面积推导核心区域尺寸。门数增生使 Core 面积从 43,171 $\mu\text{m}^2$ 膨胀至 44,221 $\mu\text{m}^2$（Die 面积由 50,868 $\mu\text{m}^2$ 扩张至 51,861 $\mu\text{m}^2$）；
   - **高扇出使能线全芯片长距离广播**: 在原始设计中，`data_valid` 仅作为流水寄存器的时钟使能；而在优化版本中，由于 `data_valid` 溶解进入了每个乘法器内部，其直接驱动的引脚数激增至 **130 多个**，网络连接数（Nets）净增 75 条（981 → 1056 条）。该信号必须横跨整个膨胀的核心区，在全芯片各个宏单元行之间进行大跨度交叉布线；
   - **局部拥塞与避让穿孔 (Congestion & Vias)**: 碎化后密集增生的 `and3_2` 与 `a21o_2` 单元造成局部引脚密度剧增。OpenROAD 详细布线器（Detailed Router）为了绕开拥塞区域并满足设计规则约束，执行了大量迂回绕线（Detours）与频繁换层，导致全芯片通孔数（Vias）从 6,514 个飙升至 7,461 个（**通孔净增 947 个，增幅 +14.5%**）；
   - **走线总长暴增 4004 $\mu\text{m}$ (+17.6%)**: 在绕线避让与高扇出跨越的叠加下，met1 走线净增 +1,995 $\mu\text{m}$，met2 净增 +1,867 $\mu\text{m}$，met3 净增 +167 $\mu\text{m}$，最终促使布线总长从 22,756 $\mu\text{m}$ 飙升至 26,760 $\mu\text{m}$（净增超 4000 $\mu\text{m}$）。

3. **互连寄生电容恶化与时钟网络功耗反噬 (Parasitic RC Overhead & Clock Net Penalty)**:
   - 暴增的 4,004 $\mu\text{m}$ 金属布线与 947 个通孔引入了极其沉重的寄生电阻与电容（RC Loading）；
   - 互连寄生与版图面积扩张增加了时钟树综合（CTS）的平衡负担，导致门控后的时钟树动态功耗由 473 $\mu\text{W}$ 逆势恶化至 510 $\mu\text{W}$（**时钟功耗反增 +37 $\mu\text{W} / +7.82\%$**）；
   - 这项额外的寄生损耗直接侵蚀了由乘法器数据门控节省的动态电量，成为 8-Tap 的净节能收益（-23.87%）未能展现高阶杠杆优势、甚至被 4-Tap（-23.76%）逼平的关键物理诱因。

4. **工程应对启示 (Design Guidelines for FIR Data Gating)**:
   - 在 RTL 编写或综合约束脚本中，应为输入数据门控信号显式添加 `(* keep = "true" *)` 属性；
   - 强制逻辑综合工具将 8-bit 与门锁定在顶层输入端口处，杜绝 ABC 优化器将其拆散溶解至算术加法树内部，从而彻底消除门数虚增与长走线反噬，使 8-Tap 恢复出超越 4-Tap 的单调上升节能效益。
