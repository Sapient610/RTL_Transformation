...
# 6. 运行小型数据通路数据门控多维全物理参数化扫描与开销反噬研究 (4b/8b/12b/16b)
...
| **`small_dp_8b`** | 8-bit 小型数据通路数据门控 | **-25.12%** (217µW → 162µW) | **+0.83%** (2562µm² → 2584µm²) | -0.45 ns | 浅逻辑微型通路在稀疏计算 (Duty=5%) 下阻断 68.8% 组合翻转，但在高频使能下存在开销反噬 |
...
## 11. 小型数据通路数据门控微观物理机理与开销反噬研究成果

通过 `script/sweep_data_gating.py` 针对 4 种微型数据通路规模（4b/8b/12b/16b）× 4 种有效使能（5%/20%/50%/80%）× 3 种总线翻转率（10%/30%/60%）共 **48 组三维全物理后仿矩阵** 进行了系统性评估（完整技术研报详见 [`doc/data_gating_study_report.md`](doc/data_gating_study_report.md)）：

### 高活跃度 (Activity = 60%) 总功耗相对变化率矩阵
| 运算规模 (Scale) | 有效计算 5% (95% 空闲) | 有效计算 20% (80% 空闲) | 有效计算 50% (50% 空闲) | 有效计算 80% (20% 空闲) | 单元增量 / 面积变化 | 关键路径延迟 (Orig → Opt) |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **4-bit 数据通路** | **-28.21%** (显著收益) | **-9.30%** (仍有收益) | <span style="color:red">**+8.29%** (开销反噬)</span> | <span style="color:red">**+7.76%** (开销反噬)</span> | +10 门 / +3.39% | 4.88 ns → 4.74 ns (+0.13 ns 改善) |
| **8-bit 数据通路** | **-39.92%** (显著收益) | **-14.48%** (显著收益) | <span style="color:red">**+8.31%** (开销反噬)</span> | <span style="color:red">**+7.84%** (开销反噬)</span> | +12 门 / +0.83% | 5.60 ns → 6.05 ns (-0.45 ns 延时) |
| **12-bit 数据通路** | **-43.88%** (大幅收益) | **-16.52%** (显著收益) | <span style="color:red">**+6.81%** (开销反噬)</span> | <span style="color:red">**+4.52%** (开销反噬)</span> | +10 门 / -0.78% | 6.63 ns → 6.90 ns (-0.27 ns 延时) |
| **16-bit 数据通路** | **-45.97%** (巨大收益) | **-17.13%** (显著收益) | <span style="color:red">**+7.99%** (开销反噬)</span> | <span style="color:red">**+6.88%** (开销反噬)</span> | +22 门 / +1.02% | 6.73 ns → 7.41 ns (-0.67 ns 延时) |

### 核心物理机理与工程判据
1. **开销反噬现象 (Overhead Inversion)**：在有效运算率较高（Valid Duty $\ge 50\%$）或外部总线安静（Activity $\le 10\%$）时，数据门控前级逻辑自身引入的动态充放电功耗，将完全反超微型计算核节省的微弱组合功耗，引发 **+4% ~ +24% 的净功耗恶化**；
2. **稀疏计算超额红利**：当有效计算率低（Duty=5%，95% 空闲）且总线噪声剧烈时，数据门控能够强力截断 **66% ~ 72%** 的组合逻辑杂散功耗，使 4b~16b 整体动态功耗锐减 **-28% 至 -46%**；
3. **时序安全裕度充裕**：由于小型数据通路组合深度极浅（3~6 级逻辑门），原始关键路径延迟仅 4.8ns ~ 6.7ns（100MHz 周期 10ns），插入门控仅增加 0.1ns ~ 0.6ns 延时，全工况零时序违例（Setup WS > +2.56ns）；
4. **架构设计决策法则**：对 4-bit 及以下极微小通路严禁无条件门控；仅在模块工作场景为稀疏突发使能（有效率 $\le 25\%$）且总线噪声大时实施。
...
- [小型数据通路数据门控影响因素与开销反噬深入研究报告](doc/data_gating_study_report.md)
