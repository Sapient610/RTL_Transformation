...
def generate_dg_study_report(
    ...
    lines.extend([
        ...
        "- **微观差异一（为什么 12-Tap > 16-Tap）**: 数据门控仅隔离乘法器而不触碰时钟树，$P_{\\text{clock}}$ 构成不可门控的底噪。16-Tap 版图面积达 91,663 um²（+42%），时钟 Sinks 增至 242 个，CTS 缓冲器暴增至 38 个（激增 2.2 倍），时钟网络功耗升至 980 uW（占整机高达 30.82%，而 12-Tap 仅占 27.78%）。根据阿姆达尔定律，不可门控的时钟底噪稀释了 16-Tap 的百分比降幅；",
        "- **微观差异二（为什么 4-Tap > 8-Tap 的物理根因）**: 8-Tap FIR 滤波常数展开为移位加法树，因未设 keep 属性，ABC 面积优化器（`amap`）将 `data_valid` 溶解渗透进所有乘法器内部进位链，将原本专用的 MUX2 (-47) 与 XOR (-40) 拆解为大量小型复合逻辑门（`and3_2` 暴增 +54、`a21o_2` 增加 +29、`a21oi_2` 增加 +19 等），致使门数反常净增 +81 门；同时版图 Core 尺寸被迫扩张，`data_valid` 全芯片长距离广播使引脚数激增至 130+，引发布线局部拥塞避让与通孔激增（Vias 净增 947 个），造成布线总长飙升 +17.6%（从 22,756 um 暴增至 26,760 um，净增超 4000 um）；最终诱发时钟网络寄生功耗反增 +37 uW（+7.82%），严重蚕食了乘法器门控省下的电量；",
        "- **结论（12-Tap 的黄金平衡点）**: 12-Tap 既充分释放了广播式单点门控的乘法器节能杠杆，又尚未触发深层 CTS 缓冲器膨胀，时钟功耗底噪占比最低（27.78%），实现了整机百分比能效的 Sweet Spot。"
    ])
