...
### 核心物理机理与工程判据
1. **时序寄存器翻转功耗全线下降**：实验证实格雷码单比特翻转特性在各规模下均能稳定降低寄存器翻转功耗（例如 4b 下时序功耗由 18.8µW 降至 17.7µW，32b 下由 135µW 降至 133µW）；
2. **次态异或逻辑膨胀与损益平衡点**：格雷码次态依赖前缀异或恢复树，导致门数急剧增加（32 位下门数由 244 增至 503 门，面积翻倍 +107.6%），组合逻辑动态功耗在 32b/80% 下从 14.2µW 暴涨至 48.2µW。本地计数器下，4 位规模呈现 -7.5% 净节能，16 位/20% 为严格损益平衡支点，8 位与 32 位则因次态异或开销反噬出现 1%~13.5% 功耗增加；
3. **真实工业选型判据**：
