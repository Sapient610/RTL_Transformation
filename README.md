...
4. **32-to-1 大规模多路选择器的非线性活跃度响应特征**：
   - *5% 准静态工况下负优化 (+2.31%)*：32 根长距离控制总线（驱动 256 个与门端子）带来的固定走线寄生与输入漏电成为固定劣势，在极低切换率下因缺乏可屏蔽的毛刺导致微量负优化；
   - *20% 轻载工况下优化落后于 8b/16b (-0.32% vs -0.78%/-1.56%)*：独热控制总线单次切换恒定双比特翻转（$1\to 0$ 和 $0\to 1$）在大版图上的充放电能量，使 32-to-1 的损益临界线向右推迟至接近 18%~20%；
   - *50%~80% 高频密集工况下爆发登顶 (-4.24% 与 -8.47%)*：二进制树 5 级对数毛刺发生雪崩，而独热 MUX 实现了 **$31/32 = 96.9\%$ 绝大多数未选通道输入跳变的入口级彻底冻结**，叠加 114 个缓冲器省除的动态红利，展现出全规模最强的节能统治力；
5. **工业选型设计判据**：
...
## 15. 总线反转编码 (Bus-Invert Coding) 物理能效与负载边界深度研究成果

针对集成电路与总线架构中经典的 Bus-Invert 低功耗编码理论，建立了**双级时钟同步流水线寄存器等量匹配（Raw Bus vs BI Bus）**且**引出工业标准外部物理 I/O Pad 接口（标称电容 $C_{\text{pad}} = 10.0\text{ pF}$，敏感度扫描 $0.05 \sim 15.0\text{ pF}$）**的基准对比体系。通过 `script/sweep_bus_invert.py` 对 4 种总线位宽规模（8b/16b/32b/64b）× 4 种输入翻转活跃度（15%/35%/60%/85%）共 **16 组全物理后仿矩阵** 展开了深度物理签核评估（完整技术研报详见 [`doc/bus_invert_study_report.md`](doc/bus_invert_study_report.md)）：

### 总功耗相对变化率全景二维矩阵 (Total Power Delta %, $C_{\text{pad}}=10.0\text{ pF}$)

![Sky130 总线反转编码各规模总功耗相对变化率](doc/images/bus_invert/bi_total_power_delta.svg)

| 总线位宽规模 | 翻转活跃度 15% | 翻转活跃度 35% | 翻转活跃度 60% (高频跳变) | 翻转活跃度 85% (反转截断) | 黄金损益平衡电容 $C_{\text{breakeven}}$ | 物理标准单元面积 (Orig → Opt) | 关键路径延迟 (Orig → Opt) | 建立时间裕量 (Setup WS) | 核心物理状态判定 |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :--- |
| **8-bit 总线** | **+2.29%** (2.18→2.23mW) | **-0.84%** (4.74→4.70mW) | **-26.47%** (7.97→5.86mW) | **-65.62%** (11.2→3.85mW) | 7.43 pF / 0.48 pF / 0.05 pF | 1061 → 1733 µm² (**+63.3%**) | 0.925 ns → 4.697 ns | 7.07 ns → **5.15 ns** | 重载下净省 7.35 mW |
| **16-bit 总线**| **+3.07%** (4.24→4.37mW) | **+0.84%** (9.49→9.57mW) | **-27.50%** (16.0→11.6mW) | **-71.70%** (22.3→6.31mW) | 15.0 pF / 0.65 pF / 0.09 pF | 2101 → 3631 µm² (**+72.8%**) | 0.936 ns → 5.219 ns | 7.06 ns → **4.65 ns** | 85% 翻转下净省 16.0 mW |
| **32-bit 总线**| **+3.35%** (8.35→8.63mW) | **+2.65%** (18.9→19.4mW) | **-27.90%** (31.9→23.0mW) | **-74.89%** (44.6→11.2mW) | >15 pF / 0.71 pF / 0.09 pF | 4238 → 7430 µm² (**+75.3%**) | 0.936 ns → 7.482 ns | 7.06 ns → **2.43 ns** | 85% 翻转下狂省 33.4 mW！ |
| **64-bit 总线**| **+4.24%** (16.5→17.2mW) | **+3.17%** (37.9→39.1mW) | **-28.62%** (63.6→45.4mW) | **-76.88%** (89.1→20.6mW) | >15 pF / 0.81 pF / 0.11 pF | 8237 → 15136 µm² (**+83.8%**) | 0.992 ns → 8.245 ns | 7.01 ns → **1.67 ns** | 64b 重载下净省 68.5 mW！ |

### 内部微观功耗拆解与外部容抗敏感度扫描

| 微观物理功耗精细拆解对比 (Toggle Rate=60%) | 外部容抗敏感度扫描与黄金损益平衡点 ($C_{\text{breakeven}}$) |
| :---: | :---: |
| ![微观功耗拆解](doc/images/bus_invert/bi_power_breakdown.svg) | ![电容敏感度扫描](doc/images/bus_invert/bi_capacitance_sensitivity.svg) |

### 动态翻转功耗相对变化率与物理面积/时序延迟权衡

| 动态翻转功耗相对变化率 (Pad Switching Delta) | 物理标准单元面积与时序延迟对比 |
| :---: | :---: |
| ![动态翻转功耗相对变化率](doc/images/bus_invert/bi_switching_reduction.svg) | ![面积与延时权衡](doc/images/bus_invert/bi_area_timing_tradeoff.svg) |

### 核心物理机理与工程判据
1. **真实 I/O Pad 驱动场景下的节能革命**：
   - 挂载 10.0 pF 工业级大电容 Pad 后，总线动态充放电功耗占据整机功耗的 96% 以上；
   - 60% 翻转率下总线动态翻转功耗稳态降低 **-27.8% ~ -30.3%**，整机功耗削减 **-26.5% ~ -28.6%**；在 85% 翻转率下反转编码将跳变率强行压缩至 15%，整机功耗断崖式暴降 **-65.6% ~ -76.9%**（64-bit 绝对节电量高达 **68.5 mW**！）；
2. **黄金损益平衡电容（$C_{\text{breakeven}}$）物理分水岭**：
   - 当翻转率 $\ge 60\%$ 时，$C_{\text{breakeven}} < 0.8\text{ pF}$（只需数百 fF 即可扭亏为盈，完全覆盖小型 Pad 与 Chiplet 凸点）；
   - 在 85% 极端翻转下，$C_{\text{breakeven}}$ 仅需 **54 ~ 110 fF**，片内跨核长线即可获得节电；
   - 在 15% 稀疏传输下，$C_{\text{breakeven}} > 15\text{ pF}$，PopCount 编码开销产生 +2% ~ +4% 的微幅反噬；
3. **时序与面积代价**：
   - 面积开销随位宽增加在 +63.3% ~ +83.8% 区间，关键路径延迟因 PopCount 加法树延长至 4.7 ~ 8.2 ns，在 100MHz 标称时钟下建立时间裕量（Setup WS $\ge 1.67\text{ ns}$）全部达标；
4. **工业选型设计黄金准则**：
   - **片内模块/执行核内部短线（$C < 0.5\text{ pF}$）**：**绝对禁止开启**，避免逻辑反噬与延时劣化；
   - **板级/DRAM/Chiplet 接口（$C > 5.0\text{ pF}$）**：**强烈推荐开启**（如 DDR4/5 DBI 模式、UCIe D2D 宽接口），在高频数据流下可获得 25% ~ 75% 的巨大系统节能并显著压制地弹噪声。
